  ...
  roles: z
    .array(
      z.object({
        ...
        scopeType: z.enum(['organization', 'building', 'room', 'department']).default('organization'),
...
export const PUT = withApi(async (request: Request, context: { params: Promise<{ id: string }> }) => {
  ...
  await withTx(ctx, async (sql) => {
    const before = await sql.query<{ role_code: string; scope_type: string; scope_id: string | null }>(
    ...
    const removingOwnSuperAdmin =
      id === admin.id && before.rows.some((r) => r.role_code === 'super_admin') && !body.roles.some((r) => r.roleCode === 'super_admin');
    if (removingOwnSuperAdmin) {
      ...
      if (Number(others.rows[0]?.count ?? 0) === 0) {
        throw new DomainError('ต้องมีผู้ดูแลระบบสูงสุดอย่างน้อยหนึ่งคน', 'last_super_admin', 409);
      }
    }
    ...
    for (const role of body.roles) {
      ...
      await sql.query(
        ...
        [id, role.roleCode, role.scopeType, role.scopeId ?? null, actor.profileId],
      );
    }
    if (body.status) {
      await sql.query('UPDATE profiles SET status = $2 WHERE id = $1', [id, body.status]);
    }
